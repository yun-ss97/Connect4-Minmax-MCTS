#
# **최종 선택된 코드에 class Rule 추가 **#
# Rule based Algorithm 으로만 동작하도록 alg() function return 값 변경
...
class Rule:
    ...
    # 3개 가로로 연속되고, 양 옆 비는 경우
    def critical(board, piece, r, c):  # piece가 바로 이기는 경우
        if 1 <= c <= 3 and board[r][c] == piece and board[r][c + 1] == piece and board[r][
            c + 2] == piece and board[r][c + 3] == 0 and board[r][c - 1] == 0:
            return True
        else:
            return False

    # CONNECT2 확인
    def winning_move2(board, piece,ROW,COL):  # 이기는 경우의 수(가로, 세로, 대각선(+/-) 상황을 모두 포함)
        # check horizontal locations for win
        ...
        # check for vertical locations for win
        for c in range(COLUMN_COUNT):
            for r in range(ROW_COUNT - 1):  # 꼭대기에서 시작할 수 없으니까 -3 해주어야 함
    ...
    # CONNECT3 확인
    def winning_move3(board, piece,ROW,COL):  # 이기는 경우의 수(가로, 세로, 대각선(+/-) 상황을 모두 포함)
            ...
            for r in range(ROW_COUNT):
                if board[r][c] == piece and board[r][c + 1] == piece and board[r][c + 2] == piece :
                    if r == ROW and c == COL:
                        ...
                        return True
                    elif r == ROW and c+2 == COL:
                        return True


        # check for vertical locations for win
        for c in range(COLUMN_COUNT):
            for r in range(ROW_COUNT - 2):  # 꼭대기에서 시작할 수 없으니까 -3 해주어야 함
                if board[r][c] == piece and board[r + 1][c] == piece and board[r + 2][c] == piece:
                    if r == ROW and c == COL:
                        return True
                    elif r+1 == ROW and c == COL:
                        return True
                    elif r+2 == ROW and c == COL:
                        return True
    ...
    # CONNECT4 확인
    def winning_move4(board, piece,ROW,COL):  # 이기는 경우의 수(가로, 세로, 대각선(+/-) 상황을 모두 포함)
        # check horizontal locations for win
        for c in range(COLUMN_COUNT - 3):
            for r in range(ROW_COUNT):
                if board[r][c] == piece and board[r][c + 1] == piece and board[r][c + 2] == piece and board[r][
    ...
    def rule(board, piece, opp_piece,round):
        col =-1
        selected = False
        valid_locations = Minmax.get_valid_locations(board)
        if round ==0 :
            ...
            return 3  # 2번째에 4열에 두기
            round +=1

        # 내가 이기는 수 두기 CONNECT4생성
        if selected ==False:
            print("RULE1. connect4 만들기")
            for c in valid_locations:
                r = get_next_open_row(board ,c)
                b_copy = board.copy()
                drop_piece(b_copy, r, c, piece)
                if Rule.winning_move4(b_copy, piece,r,c):
                    selected = True
                    col = c
                    break

        # 상대가 이기는 수 막기 CONNECT4생성
        if selected == False:
            print("RULE2. connect4 막기")
            for c in valid_locations:
                r = get_next_open_row(board ,c)
                b_copy = board.copy()
                drop_piece(b_copy, r, c, opp_piece)
                if Rule.winning_move4(b_copy, opp_piece,r,c):
                    selected = True
                    col = c
                    break


        if selected == False:  # 내(AI) critical 두기
            print("RULE2.1 critical 만들기")
            for c in valid_locations:
                r = get_next_open_row(board, c)
                b_copy = board.copy()
                drop_piece(b_copy, r, c, piece)
                if r == 0 and Rule.critical(b_copy, piece, r,c):
                    selected = True
                    col = c
                    break

        if selected == False:  # 상대 critical 두기
            print("RULE2.2 critical 막기")
            for c in valid_locations:
                r = get_next_open_row(board, c)
                b_copy = board.copy()
                drop_piece(b_copy, r, c, opp_piece)
                if r == 0 and Rule.critical(b_copy, opp_piece, r, c):
                    selected = True
                    col = c
                    break

        if selected == False:# 내가 CONNECT3 생성
            print("RULE3. connect3 만들기")
            for c in valid_locations:
                r = get_next_open_row(board ,c)
                b_copy = board.copy()
                drop_piece(b_copy, r, c, piece)
                if Rule.winning_move3(b_copy, piece,r,c):
                    selected = True
                    col = c
                    break

        if selected == False:  # 상대 CONNECT3 막기
            print("RULE4. connect3 막기")
            for c in valid_locations:
                r = get_next_open_row(board ,c)
                b_copy = board.copy()
                drop_piece(b_copy, r, c, opp_piece)
                if Rule.winning_move3(b_copy, opp_piece,r,c):
                    selected = True
                    col = c
                    break
        if selected == False:  # 내가 CONNECT2 생성
            print("RULE5. connect2 만들기")

            for c in valid_locations:
                r = get_next_open_row(board ,c)
                b_copy = board.copy()
                drop_piece(b_copy, r, c, piece)
                if Rule.winning_move2(b_copy, piece,r,c):
                    selected = True
                    col = c
                    break
        if selected == False: # 상대 CONNECT2 막기
            print("RULE6. connect2 막기")

            for c in valid_locations:
                r = get_next_open_row(board ,c)
                b_copy = board.copy()
                drop_piece(b_copy, r, c, opp_piece)
                if Rule.winning_move2(b_copy, opp_piece,r,c):
                    selected = True
                    col = c
                    break
        elif selected ==False:
            print("RULE7. 가운데에 가까운 열에 놓기")
            for c in valid_locations:
                distance = list(map(lambda x: abs(x - 3), valid_locations))  # colum4,즉 index 3과의 거리 계산 (가까울 수록 가운데 인것)
                shortest_distance = sorted(distance)[0]  # 오름차순 정렬의 첫 번째값
                shortest_loc = distance.index(shortest_distance)
                col = valid_locations[shortest_loc]  # 최단 거리가 중복일 경우 맨 왼쪽값 하나만 return
        return col
...
while not game_over:  # turn 의 나머지가 0이면 PLAYER 차례이고, 나머지가 1이면 AI 차례임.
    ...
    # Ask for player 2 input
        ...
        if alg() == 1:
            ...
            print("선택된 방법은 Rule-based Algorithm 입니다.")

            start_time = time.time()
            col = Rule.rule(board, AI_PIECE, PLAYER_PIECE,round)
            print("---- %.2f seconds ----" % (time.time() - start_time))  # 시간 체크)
            ...
            print_board(board)
            print("[AI CHOICE] Rule에 의해 column %d 를 선택함" % (col + 1))
